...
TwoWire *WireVL53L0X;		//0:Wire, 1:Wire1, 2:Wire2, 3:Wire3 4:Wire4 5:Wire5
...
{
	WireVL53L0X->beginTransmission(address);
	WireVL53L0X->write(reg);
	WireVL53L0X->write(value);
	last_status = WireVL53L0X->endTransmission();
}
...
void VL53L0X::writeReg16Bit(uint8_t reg, uint16_t value)
{
	WireVL53L0X->beginTransmission(address);
	WireVL53L0X->write(reg);
	WireVL53L0X->write((value >> 8) & 0xFF); // value high byte
	WireVL53L0X->write(value & 0xFF); // value low byte
	last_status = WireVL53L0X->endTransmission();
}
...
void VL53L0X::writeReg32Bit(uint8_t reg, uint32_t value)
{
	WireVL53L0X->beginTransmission(address);
	WireVL53L0X->write(reg);
	WireVL53L0X->write((value >> 24) & 0xFF); // value highest byte
	WireVL53L0X->write((value >> 16) & 0xFF);
	WireVL53L0X->write((value >> 8) & 0xFF);
	WireVL53L0X->write(value & 0xFF); // value lowest byte
	last_status = WireVL53L0X->endTransmission();
}
...
uint8_t VL53L0X::readReg(uint8_t reg)
{
	...
	WireVL53L0X->requestFrom(address, (uint8_t)1);
	value = WireVL53L0X->read();
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg)
{
	...
	WireVL53L0X->requestFrom(address, (uint8_t)2);
	value = (uint16_t)WireVL53L0X->read() << 8; // value high byte
	value |= WireVL53L0X->read();      // value low byte
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
	...
	WireVL53L0X->requestFrom(address, (uint8_t)4);
	value = (uint32_t)WireVL53L0X->read() << 24; // value highest byte
	value |= (uint32_t)WireVL53L0X->read() << 16;
	value |= (uint16_t)WireVL53L0X->read() << 8;
	value |= WireVL53L0X->read();       // value lowest byte
...
void VL53L0X::readMulti(uint8_t reg, uint8_t * dst, uint8_t count)
{
	...
	WireVL53L0X->requestFrom(address, count);

	while (count-- > 0)
	{
		*(dst++) = WireVL53L0X->read();
	}
}
