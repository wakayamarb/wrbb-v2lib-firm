...
int fileloader(const char* str0, const char* str1)
{
	...
	while (true) {
		...
		if (CommandData[0] == 'Z') {
		...
		else if (CommandData[0] == 'R' || CommandData[0] == 'N') {

			IgnoreOP_STOP = false;
			if (CommandData[0] == 'N') { IgnoreOP_STOP = true; }

			if (strlen(CommandData) == 1) {
				strcpy(&CommandData[1], " main");
			}
			else if (strlen(CommandData) == 2) {
				strcpy(&CommandData[1], "main");
			}
			...
			USB_Serial->println();
			USB_Serial->println("EEPROM FileWriter Ver. 1.79.v2");
			...
			USB_Serial->println(" N:Run File Ignore break..>N Filename [ENTER]");
